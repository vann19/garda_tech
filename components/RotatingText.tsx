...
export interface RotatingTextProps extends Omit<HTMLMotionProps<'span'>, 'children' | 'initial' | 'animate' | 'exit' | 'transition'> {
  texts: string[];
  transition?: any;
  initial?: any;
  animate?: any;
  ...
  if (!mounted) {
    return (
      <span className={cn('text-rotate', mainClassName)} {...(rest as any)}>
        <span className="text-rotate-word">{texts[0]}</span>
      </span>
    );
  }
